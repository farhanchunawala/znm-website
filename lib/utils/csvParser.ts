...
export function parseCSV(csvContent: string): CSVOrderRow[] {
	...
	for (let i = 1; i < lines.length; i++) {
		...
		rows.push({
			...
			itemTitle:
				row.itemtitle || row.item_title || row.item || row.product,
			itemQuantity: parseInt(
				row.itemquantity || row.item_quantity || row.quantity || '1'
			),
			itemSize: row.itemsize || row.item_size || row.size || 'M',
			itemPrice: parseFloat(
				row.itemprice || row.item_price || row.price || '0'
			),
			paymentStatus: (
				row.paymentstatus ||
				row.payment_status ||
				'unpaid'
			).toLowerCase() as 'prepaid' | 'unpaid',
			notes: row.notes,
		});
	}
...
export function validateOrderRow(row: CSVOrderRow): {
	valid: boolean;
	errors: string[];
} {
	const errors: string[] = [];
	...
	if (!row.itemQuantity || row.itemQuantity <= 0)
		errors.push('Valid item quantity is required');

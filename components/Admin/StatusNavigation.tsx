...
export default function StatusNavigation({
	...
	const [modalType, setModalType] = useState<'forward' | 'backward'>(
		'forward'
	);
	...
	const handleConfirm = async (data?: any) => {
		...
		try {
			const targetStatus =
				modalType === 'forward' ? nextStatus : previousStatus;
			if (targetStatus) {
			...
			<div className={styles.statusNavigation}>
				...
				<div className={styles.currentStatus}>
					{STATUS_ICONS[currentStatus] &&
						React.createElement(STATUS_ICONS[currentStatus], {
							size: 24,
						})}
...
function TransitionModal({
	...
	const [packagingProvider, setPackagingProvider] = useState<
		'we' | 'courier'
	>('we');
	const [courierName, setCourierName] = useState('');

	const needsPackagingInfo =
		currentStatus === 'fulfilled' && targetStatus === 'shipped';
	const needsCourierOnly =
		currentStatus === 'shipped' && targetStatus === 'outForDelivery';
	...
	return (
		<div className={styles.modalOverlay} onClick={onCancel}>
			<div className={styles.modal} onClick={(e) => e.stopPropagation()}>
				<h2>
					{modalType === 'forward' ? 'Forward' : 'Move Back'} to{' '}
				...
				{needsPackagingInfo && (
					<div className={styles.modalContent}>
						...
						<p className={styles.note}>
							This will generate an invoice and send it to the
							customer via email.
						</p>
					</div>
				)}
				...
				{!needsPackagingInfo && !needsCourierOnly && (
					<div className={styles.modalContent}>
						<p>
							...
							{modalType === 'forward' ? 'forward' : 'move back'}{' '}
							this {entityType} to{' '}
							<strong>{STATUS_LABELS[targetStatus]}</strong>?
						</p>
						{targetStatus === 'fulfilled' && (
							<p className={styles.note}>
								This will send an order fulfilled email to the
								customer.
							</p>
						)}
						{targetStatus === 'delivered' && (
							<p className={styles.note}>
								This will send a delivery confirmation email
								with feedback link.
							</p>
						)}
					</div>

...
export default function NewOrderPage() {
	...
	const [paymentStatus, setPaymentStatus] = useState<'prepaid' | 'unpaid'>(
		'unpaid'
	);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (
			items.some((item) => !item.title || !item.size || item.price <= 0)
		) {
				...
				<div className={styles.section}>
					...
					<div className={styles.paymentRow}>
						...
						<select
							...
							onChange={(e) =>
								setPaymentStatus(
									e.target.value as 'prepaid' | 'unpaid'
								)
							}

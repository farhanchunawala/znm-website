...
export async function POST(request: NextRequest) {
	try {
		...
		if (existingCustomer) {
			...
			if (
				existingCustomer.emails &&
				!Array.isArray(existingCustomer.emails)
			) {
			...
			console.log(
				'Update complete. Current emails:',
				updatedCustomer?.emails
			);
		} else {
			...
			const latestCustomer = await Customer.findOne()
				...
				.exec();
			const lastNum =
				latestCustomer && latestCustomer.customerId
					? parseInt(
							latestCustomer.customerId.replace('znm-', ''),
							10
						)
					: 0;
		...
		const lastOrderNum =
			latestOrder && latestOrder.orderId
				? parseInt(latestOrder.orderId.replace('#znmon', ''), 10)
				: 0;
